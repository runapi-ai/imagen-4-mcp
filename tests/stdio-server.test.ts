...
describe("imagen-4 stdio MCP server", () => {
  ...
  it("exposes the model-line tools over the real stdio transport", async () => {
    ...
    client = new Client({ name: "imagen-4-mcp-test", version: "0.1.0" });
    ...
    expect(names).toEqual(["check_pricing","get_task","login","remix_image","text_to_image"]);
    ...
    for (const model of ["imagen-4-pro-remix-image","imagen-4","imagen-4-fast","imagen-4-ultra"]) {
